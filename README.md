...
| Path | 説明 |
|------|------|
| `docs/00_project/` | 憲章・ロードマップ |
| `docs/10_ast/` | AST 設計成果物 |
| `docs/20_ir/` | IR 定義・写像メモ |
| `docs/30_cfg/` | CFG モデリングメモ |
| `docs/40_dfg/` | DFG モデリングメモ |
| `docs/50_guarantee/` | 移行保証理論 |
| `docs/90_glossary/` | 用語定義 |
| `docs/prompts/` | 研究ワークフロー用プロンプト資産 |
| `docs/prompts/research/exec/` | 実行可能プロンプト（研究ログ生成など） |
| `diagrams/mermaid/` | Mermaid ソース |
| `log/` | 研究・日次メモのルート |
| `log/research-log/` | 研究ログ・テンプレート |
| `log/working-log/` | 作業ログ |
| `samples/cobol/` | 解析用 COBOL サンプル |
| `tools/` | ユーティリティ・補助ツール |
...
| `docs/prompts/` | Prompt assets for structured research workflow |
| `docs/prompts/research/exec/` | Executable prompt definitions (e.g. research log generation) |
| `diagrams/mermaid/` | Mermaid source diagrams |
| `log/` | Canonical logging root for research and memos |
| `log/research-log/` | Research decision logs and templates |
| `log/working-log/` | Daily working memos |
