...
## 🎯 今日の研究焦点（1つだけ）
- Phase 6 の第7文書として、変更影響を **Change Origin** から始まる **structural propagation** として捉え、その到達領域を **Impact Scope** として形式化する。
...
- **Local Impact Scope** と **Extended Impact Scope** を分けないと、局所的には正しいが移行判断としては不十分な分析が生じる。
...
- **Local / Extended Impact Scope** を区別し、\( T_{loc}(o) \subseteq T_{ext}(o) \) の関係で整理した。
...
## ⚠ 保留・未解決
- `B_{imp}` を、外部露出・未閉包・ブラックボックス境界としてどこまで厳密に分類するかは未確定である。
